...
def main():
    ...
    # Hardcoded name mapping (IDs and names won't change)
    HARDCODED_NAME_MAP = {
    ...
    # Step 1: Scrape all competitors (19 requests only)
    print("Scraping competition data...")
    competitors = []
    total = len(HARDCODED_NAME_MAP)
    
    for i, (public_id, name) in enumerate(HARDCODED_NAME_MAP.items(), 1):
        print(f"Scraping competitor {i}/{total}: {name}")
        
        # Use dummy rank for now, will calculate real ranks after
        competitor = scraper.scrape_competitor(public_id, 0, HARDCODED_NAME_MAP)
        if competitor:
            competitors.append(competitor)
            print(f"  ✓ {competitor.name} - ${competitor.portfolio_value:,.2f}")
        else:
    ...
    # Step 2: Calculate actual ranks based on portfolio values
    print("\nCalculating rankings...")
    competitors.sort(key=lambda x: x.portfolio_value, reverse=True)
